...
1. **Automated RFP & Security Questionnaire Filling**
   - Provide Sellora with a massive block of RFP text. It uses Groq/OpenAI to extract every individual question, embeds them using a local Ollama model (`all-minilm`), queries your company's uploaded documents via `pgvector`, and returns perfectly cited answers for every question.
...
- **ORM:** Prisma
- **AI / Embeddings:** 
  - **Local Model (Privacy-First):** Ollama daemon running `all-minilm` (384-dimensions) for fast, free local embeddings.
...
docker compose up -d
```
*(Note: Ollama will automatically pull the `all-minilm` model on startup).*
