...
## Task: Next Backend Build-Out

Created the next MVP backend pieces from the remaining roadmap: workflow worker utilities, CRM writeback preparation, document ingestion foundation, and a test harness.

## Changes Made

- Added CRM writeback preparation under `/api/v1/crm-writeback`.
- Added document metadata and text-ingestion routes under `/api/v1/documents`.
- Added workflow queue processing helpers and admin routes.
- Added a lightweight workflow worker script for polling queued workflow runs.
- Added grounded answer/RAG MVP under `/api/v1/ai/grounded-answers`.
- Added Node test runner setup with focused schema/normalization tests.
- Registered the new modules in the main API router.
- Added worker/test/typecheck scripts to the server package.

## New/Updated Routes

### `POST /api/v1/crm-writeback/preview`

JWT-protected admin/manager route.

Creates a pending `SyncLog` containing a provider-ready CRM writeback payload from a completed workflow run.

Current provider shaping:

- Salesforce: `fields`
- HubSpot: `properties`
- Other providers: generic Sellora payload

### `GET /api/v1/documents`

Lists documents for the authenticated user's organization.
...
Creates document metadata using the existing `Document` Prisma model.

### `GET /api/v1/documents/:id`

Returns one org-scoped document.

### `POST /api/v1/documents/:id/ingest-text`
...
- chunks plain text
- stores rows in `DocumentChunk`
- marks the document `COMPLETED`
- uses local placeholder Pinecone IDs until real embedding/vector storage is connected

### `POST /api/v1/workflow-runs/process-next`

Processes the oldest queued `SALES_EVENT_EXTRACTION` workflow run for the authenticated organization.

### `POST /api/v1/workflow-runs/process-queued`

Processes up to `limit` queued `SALES_EVENT_EXTRACTION` workflow runs for the authenticated organization.

### `POST /api/v1/ai/grounded-answers`

JWT-protected route for asking questions against completed document chunks.

MVP behavior:

- retrieves org-scoped completed document chunks
- scores chunks using local keyword overlap
- returns citations with document/chunk metadata
- refuses when no relevant grounded context is found
- stores an `AiInteraction` audit row

This is an extractive local RAG MVP. OpenAI/Pinecone-backed retrieval and generation are still future upgrades.
...
```bash
tsx src/workers/workflowWorker.ts
```

It polls queued workflow runs using:
...
### `apps/server/src/modules/crmWriteback/*`

Schema, service, controller, and router for preparing CRM writeback payloads and recording pending outbound sync logs.

### `apps/server/src/modules/documents/*`

Schema, service, controller, and router for document metadata, listing, detail, and MVP text ingestion.

### `apps/server/src/modules/groundedAnswers/*`

Schema, service, controller, and router for grounded document Q&A with citations and refusal behavior.

### `apps/server/src/workers/workflowWorker.ts`

Polling worker for queued workflow runs.

### Test Files

- `apps/server/src/utils/__tests__/aiExtraction.test.ts`
- `apps/server/src/modules/documents/documents.schema.test.ts`
- `apps/server/src/modules/crmWriteback/crmWriteback.schema.test.ts`
- `apps/server/src/modules/groundedAnswers/groundedAnswers.schema.test.ts`
...
### `apps/server/src/modules/workflowRuns/*`

Added queue-processing service methods, controllers, schemas, and routes.
...
Added workflow worker settings.
...
- TypeScript compilation passed.
- Node test runner passed all tests.
- Prisma schema validation passed.

## Notes

- Document ingestion currently stores chunks locally and uses placeholder Pinecone IDs. Real embeddings/vector upsert are still next.
- Workflow worker currently processes supported `SALES_EVENT_EXTRACTION` and `GROUNDED_TECHNICAL_ANSWER` runs.
- CRM writeback preparation creates a pending `SyncLog`; n8n or a future outbound sync worker should perform the actual Salesforce/HubSpot API call.
- Grounded answers are currently extractive/local. Real embeddings, vector search, and AI-generated cited answers remain the next RAG upgrades.
